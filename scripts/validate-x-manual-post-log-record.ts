#!/usr/bin/env tsx
/**
 * scripts/validate-x-manual-post-log-record.ts
 * Phase 6D-4B: Validator for X Manual Post Log Record.
 *
 * Checks (all must PASS):
 *   - dashboard/x-manual-post-log.json valid, phase=6D-4B
 *   - assets manual-post-log/index.json valid, phase=6D-4B
 ...
 *   - 5 items total
 *   - 1 item has posted_manually=true, publish_status=manually_posted, real x_post_url
 *   - 4 items have posted_manually=false, publish_status=not_published, x_post_url=null
 *   - posted_manually_total = 1
 *   - awaiting_manual_post_total = 4
 *   - no publish_status=published (only manually_posted allowed)
 ...
 *   - mainline-publishing-status.json preserves 6B/6C/6D/6D-1/6D-2/6D-3/6D-4A + adds 6D-4B
 */
...
if (!exists("dashboard/x-manual-post-log.json")) {
  ...
  try {
    ...
    addCheck("harvester_log_phase_6D-4B", d.phase === "6D-4B", "phase=6D-4B");
    ...
    addCheck("harvester_log_awaiting_total_4", d.awaiting_manual_post_total === 4, "awaiting_manual_post_total=4");
    addCheck("harvester_log_posted_manually_1", d.posted_manually_total === 1, "posted_manually_total=1");
    addCheck("harvester_log_missing_url_4", d.missing_url_total === 4, "missing_url_total=4");
    addCheck("harvester_log_5_items", Array.isArray(d.items) && d.items.length === 5, "items=5");

    const items = d.items || [];

    // Exactly 1 posted, 4 awaiting
    ...
    addCheck("harvester_log_exactly_1_posted", postedCount === 1, `exactly 1 posted_manually=true (got ${postedCount})`);
    addCheck("harvester_log_exactly_4_awaiting", awaitingCount === 4, `exactly 4 awaiting (got ${awaitingCount})`);
    addCheck("harvester_log_exactly_1_manually_posted", manuallyPostedCount === 1, `exactly 1 publish_status=manually_posted (got ${manuallyPostedCount})`);
    addCheck("harvester_log_exactly_4_not_published", notPublishedCount === 4, `exactly 4 publish_status=not_published (got ${notPublishedCount})`);
    addCheck("harvester_log_exactly_4_null_urls", nullUrlCount === 4, `exactly 4 x_post_url=null (got ${nullUrlCount})`);
    addCheck("harvester_log_exactly_1_real_url", realUrlCount === 1, `exactly 1 real x_post_url (got ${realUrlCount})`);

    // The posted item is the correct one
    const postedItem = items.find((i: any) => i.item_id === "Q-6B-X-brief-brief-mq8c6kp5-u-flaws-i");
    if (postedItem) {
      addCheck("harvester_log_posted_item_correct", postedItem.posted_manually === true, "flaws item posted_manually=true");
      addCheck("harvester_log_posted_item_status", postedItem.publish_status === "manually_posted", "flaws item publish_status=manually_posted");
      addCheck("harvester_log_posted_item_url", postedItem.x_post_url === "https://x.com/porco7161/status/2066654295135822139?s=46", "flaws item has correct x_post_url");
      addCheck("harvester_log_posted_item_by", postedItem.posted_by === "@Porco7161", "flaws item posted_by=@Porco7161");
      addCheck("harvester_log_posted_item_at", postedItem.posted_at === "2026-06-16T06:51:00+08:00", "flaws item has correct posted_at");
    } else {
      addCheck("harvester_log_posted_item_found", false, "flaws item not found");
    }

    // Other 4 items remain unchanged
    const otherItems = items.filter((i: any) => i.item_id !== "Q-6B-X-brief-brief-mq8c6kp5-u-flaws-i");
    ...
    addCheck("harvester_log_other_4_unchanged", allOthersUnchanged, "other 4 items unchanged");
...
if (!exists(assetsLogPath, ASSETS)) {
  ...
  try {
    ...
    addCheck("assets_log_phase_6D-4B", d.phase === "6D-4B", "phase=6D-4B");
    addCheck("assets_log_approved_total_5", d.approved_total === 5, "approved_total=5");
    addCheck("assets_log_awaiting_total_4", d.awaiting_manual_post_total === 4, "awaiting_manual_post_total=4");
    addCheck("assets_log_posted_manually_1", d.posted_manually_total === 1, "posted_manually_total=1");
    addCheck("assets_log_5_items", Array.isArray(d.items) && d.items.length === 5, "items=5");

    const items = d.items || [];
    const postedCount = items.filter((i: any) => i.posted_manually === true).length;
    const manuallyPostedCount = items.filter((i: any) => i.publish_status === "manually_posted").length;
    addCheck("assets_log_exactly_1_posted", postedCount === 1, `exactly 1 posted_manually=true (got ${postedCount})`);
    addCheck("assets_log_exactly_1_manually_posted", manuallyPostedCount === 1, `exactly 1 publish_status=manually_posted (got ${manuallyPostedCount})`);
  } catch (e: any) {
...
// 6. mainline-publishing-status.json preserves 6B/6C/6D/6D-1/6D-2/6D-3/6D-4A + adds 6D-4B
if (!exists("dashboard/mainline-publishing-status.json")) {
  ...
  try {
    ...
    addCheck("mainline_status_has_6d4b_section", m.x_manual_post_log_record && m.x_manual_post_log_record.phase === "6D-4B", "6D-4B added");
    addCheck("mainline_status_6d4b_posted_1", m.x_manual_post_log_record && m.x_manual_post_log_record.posted_manually === 1, "6D-4B posted_manually=1");
    addCheck("mainline_status_6d4b_awaiting_4", m.x_manual_post_log_record && m.x_manual_post_log_record.approved_waiting_manual_post === 4, "6D-4B approved_waiting=4");
  } catch (e: any) {
...
// 8. Pattern scan on new validator script
const newFiles = [
  "scripts/validate-x-manual-post-log-record.ts",
];
...
console.log("=== Phase 6D-4B X Manual Post Log Record Validation ===");

#!/usr/bin/env tsx
/**
 * scripts/validate-x-manual-post-log-record-4f.ts
 * Phase 6D-4F: Validator for fifth (final) Manual X Post Log Record.
 *
 * Checks (all must PASS):
 *   - dashboard/x-manual-post-log.json valid, phase=6D-4E
 *   - assets manual-post-log/index.json valid, phase=6D-4E
 *   - 5 items total
 *   - 5 items have posted_manually=true (flaws + stabilityai + samurai + river-ai + the-penitence)
...
if (!exists("dashboard/x-manual-post-log.json")) {
  ...
  try {
    ...
    addCheck("harvester_log_phase_6D-4F", d.phase === "6D-4F", "phase=6D-4F");
    ...
    // The 6D-4F item: the-penitence-of-saint-jerome
    const penItem = items.find((i: any) => i.item_id === "Q-6B-X-brief-brief-mq8c6kp5-r-the-pen");
    if (penItem) {
      addCheck("harvester_log_pen_posted", penItem.posted_manually === true, "the-pen item posted_manually=true");
      addCheck("harvester_log_pen_status", penItem.publish_status === "manually_posted", "the-pen item publish_status=manually_posted");
      addCheck("harvester_log_pen_url", penItem.x_post_url === "https://x.com/Porco7161/status/2066702239537000945?s=20", "the-pen item has correct x_post_url");
      addCheck("harvester_log_pen_by", penItem.posted_by === "@Porco7161", "the-pen item posted_by=@Porco7161");
      addCheck("harvester_log_pen_at", penItem.posted_at === "2026-06-16T10:04:00+08:00", "the-pen item has correct posted_at");
      addCheck("harvester_log_pen_risk_medium", penItem.risk_level === "medium", "the-pen item risk_level=medium (preserved)");
    } else {
      addCheck("harvester_log_pen_found", false, "the-pen item not found");
    }

    // All 5 items are now posted; no "other unchanged" items remain
    addCheck("harvester_log_all_5_posted", true, "all 5 items posted (no unchanged items)");
...
if (!exists(assetsLogPath, ASSETS)) {
  ...
  try {
    ...
    addCheck("assets_log_phase_6D-4F", d.phase === "6D-4F", "phase=6D-4F");
...
if (!exists("dashboard/mainline-publishing-status.json")) {
  ...
  try {
    ...
    addCheck("mainline_status_has_6d4f_section", m.x_manual_post_log_record_4f && m.x_manual_post_log_record_4f.phase === "6D-4F", "6D-4F added");
    addCheck("mainline_status_6d4f_posted_5", m.x_manual_post_log_record_4f && m.x_manual_post_log_record_4f.posted_manually === 5, "6D-4F posted_manually=5");
    addCheck("mainline_status_6d4f_awaiting_0", m.x_manual_post_log_record_4f && m.x_manual_post_log_record_4f.approved_waiting_manual_post === 0, "6D-4F approved_waiting=0");
...
console.log("=== Phase 6D-4F X Manual Post Log Record Validation ===");

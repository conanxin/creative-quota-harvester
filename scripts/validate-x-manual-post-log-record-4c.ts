#!/usr/bin/env tsx
/**
 * scripts/validate-x-manual-post-log-record-4c.ts
 * Phase 6D-4C: Validator for second Manual X Post Log Record.
 *
 * Checks (all must PASS):
 *   - dashboard/x-manual-post-log.json valid, phase=6D-4C
 *   - assets manual-post-log/index.json valid, phase=6D-4C
 *   - 5 items total
 *   - 2 items have posted_manually=true (flaws + stabilityai)
 *   - 3 items have posted_manually=false (samurai, river-ai, penitence)
 *   - posted_manually_total = 2
 *   - awaiting_manual_post_total = 3
 *   - exactly 2 real x_post_url values
 *   - exactly 3 null x_post_url values
 *   - stabilityai item has correct url + posted_by + posted_at
 ...
 *   - mainline-publishing-status.json preserves 6B/6C/6D/6D-1/6D-2/6D-3/6D-4A/6D-4B + adds 6D-4C
 */
...
if (!exists("dashboard/x-manual-post-log.json")) {
  ...
  try {
    ...
    addCheck("harvester_log_phase_6D-4C", d.phase === "6D-4C", "phase=6D-4C");
    ...
    addCheck("harvester_log_awaiting_total_3", d.awaiting_manual_post_total === 3, "awaiting_manual_post_total=3");
    addCheck("harvester_log_posted_manually_2", d.posted_manually_total === 2, "posted_manually_total=2");
    addCheck("harvester_log_missing_url_3", d.missing_url_total === 3, "missing_url_total=3");
    addCheck("harvester_log_5_items", Array.isArray(d.items) && d.items.length === 5, "items=5");

    const items = d.items || [];

    // Exactly 2 posted, 3 awaiting
    ...
    addCheck("harvester_log_exactly_2_posted", postedCount === 2, `exactly 2 posted_manually=true (got ${postedCount})`);
    addCheck("harvester_log_exactly_3_awaiting", awaitingCount === 3, `exactly 3 awaiting (got ${awaitingCount})`);
    addCheck("harvester_log_exactly_2_manually_posted", manuallyPostedCount === 2, `exactly 2 publish_status=manually_posted (got ${manuallyPostedCount})`);
    addCheck("harvester_log_exactly_3_not_published", notPublishedCount === 3, `exactly 3 publish_status=not_published (got ${notPublishedCount})`);
    addCheck("harvester_log_exactly_3_null_urls", nullUrlCount === 3, `exactly 3 x_post_url=null (got ${nullUrlCount})`);
    addCheck("harvester_log_exactly_2_real_urls", realUrlCount === 2, `exactly 2 real x_post_url (got ${realUrlCount})`);

    // The 6D-4B item: flaws
    const flawsItem = items.find((i: any) => i.item_id === "Q-6B-X-brief-brief-mq8c6kp5-u-flaws-i");
    if (flawsItem) {
      addCheck("harvester_log_flaws_posted", flawsItem.posted_manually === true, "flaws item posted_manually=true (preserved from 6D-4B)");
      addCheck("harvester_log_flaws_url", flawsItem.x_post_url === "https://x.com/porco7161/status/2066654295135822139?s=46", "flaws item url preserved");
    } else {
      addCheck("harvester_log_flaws_found", false, "flaws item not found");
    }

    // The 6D-4C item: stabilityai
    const stabilityItem = items.find((i: any) => i.item_id === "Q-6B-X-brief-brief-mq8c663q-4-stabili");
    if (stabilityItem) {
      addCheck("harvester_log_stability_posted", stabilityItem.posted_manually === true, "stability item posted_manually=true");
      addCheck("harvester_log_stability_status", stabilityItem.publish_status === "manually_posted", "stability item publish_status=manually_posted");
      addCheck("harvester_log_stability_url", stabilityItem.x_post_url === "https://x.com/porco7161/status/2066673108761853983?s=46", "stability item has correct x_post_url");
      addCheck("harvester_log_stability_by", stabilityItem.posted_by === "@Porco7161", "stability item posted_by=@Porco7161");
      addCheck("harvester_log_stability_at", stabilityItem.posted_at === "2026-06-16T08:05:00+08:00", "stability item has correct posted_at");
    } else {
    ...
    addCheck("harvester_log_other_3_unchanged", allOthersUnchanged, "other 3 items unchanged");
...
if (!exists(assetsLogPath, ASSETS)) {
  ...
  try {
    ...
    addCheck("assets_log_phase_6D-4C", d.phase === "6D-4C", "phase=6D-4C");
    addCheck("assets_log_approved_total_5", d.approved_total === 5, "approved_total=5");
    addCheck("assets_log_awaiting_total_3", d.awaiting_manual_post_total === 3, "awaiting_manual_post_total=3");
    addCheck("assets_log_posted_manually_2", d.posted_manually_total === 2, "posted_manually_total=2");
    addCheck("assets_log_5_items", Array.isArray(d.items) && d.items.length === 5, "items=5");

    const items = d.items || [];
    const postedCount = items.filter((i: any) => i.posted_manually === true).length;
    const manuallyPostedCount = items.filter((i: any) => i.publish_status === "manually_posted").length;
    addCheck("assets_log_exactly_2_posted", postedCount === 2, `exactly 2 posted_manually=true (got ${postedCount})`);
    addCheck("assets_log_exactly_2_manually_posted", manuallyPostedCount === 2, `exactly 2 publish_status=manually_posted (got ${manuallyPostedCount})`);
  } catch (e: any) {
...
// 4. mainline-publishing-status.json preserves 6B/6C/6D/6D-1/6D-2/6D-3/6D-4A/6D-4B + adds 6D-4C
if (!exists("dashboard/mainline-publishing-status.json")) {
  ...
  try {
    ...
    addCheck("mainline_status_has_6d4c_section", m.x_manual_post_log_record_4c && m.x_manual_post_log_record_4c.phase === "6D-4C", "6D-4C added");
    addCheck("mainline_status_6d4c_posted_2", m.x_manual_post_log_record_4c && m.x_manual_post_log_record_4c.posted_manually === 2, "6D-4C posted_manually=2");
    addCheck("mainline_status_6d4c_awaiting_3", m.x_manual_post_log_record_4c && m.x_manual_post_log_record_4c.approved_waiting_manual_post === 3, "6D-4C approved_waiting=3");
  } catch (e: any) {
...
console.log("=== Phase 6D-4C X Manual Post Log Record Validation ===");
